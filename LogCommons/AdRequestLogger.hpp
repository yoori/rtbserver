...
public:
  ...
  const OptionalUlong&
  device_channel_id() const throw ()
  {
    return device_channel_id_;
  }

  const OptionalValue<RequestId>&
  action_request_id() const throw ()
  {
  ...
  referrer() const throw ()
  {
    return referrer_;
  }

  const StringIoWrapperOptional&
  order_id() const throw ()
  {
    return order_id_;
  }

  const StringIoWrapperOptional&
  ip_address() const throw ()
  {
    return ip_address_;
  }

  const FixedNumber&
  cur_value() const throw ()
  {
    return cur_value_;
  }

  AdvertiserActionData_V_3_3_1() throw () : cur_value_(FixedNumber::ZERO) {}
...
public:
  ...
  AdvertiserActionData() throw () : cur_value_(FixedNumber::ZERO) {}
