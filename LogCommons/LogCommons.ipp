...
struct DefaultSequenceAdapter
{
  template <class T>
  const T
  operator()(const T& value) const throw()
  {
    return value;
  }
};
...
public:
  ...
  void
  setstate(const std::ios_base::iostate& state) throw ()
  {
    iostate_ |= state;
  }
  ...
  void
  take_fails(const FixedBufStream<SomeCategory>& stream) throw ()
  {
    iostate_ |=
  ...
  void
  transfer_state(std::basic_ios<Char, Traits>& stream) const throw ()
  {
...
{
public:
  TokenizerInputArchive(FixedBufStream<Category>& stream) throw ()
...
{
public:
  InputArchive(std::istream& istr) throw ()
    : istr_(istr)
  {}
...
{
public:
  OutputArchive(std::ostream& ostr) throw ()
    : ostr_(ostr)
  {}
