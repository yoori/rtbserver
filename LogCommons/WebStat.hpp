...
public:
  ...
  const std::string& ct() const throw()
  {
    return ct_.get();
  }

  const std::string& curct() const throw()
  {
    return curct_.get();
  }

  const std::string& browser() const throw()
  {
    return browser_.get();
  }

  const std::string& os() const throw()
  {
    return os_.get();
  }

  unsigned long
  web_operation_id() const throw()
  {
    return web_operation_id_;
  }

  char result() const throw()
  {
    return result_;
  }

  char user_status() const throw()
  {
    return user_status_;
  }
  ...
  const OptionalUlong& tag_id() const throw()
  {
    return tag_id_;
  }

  const OptionalUlong& cc_id() const throw()
  {
    return cc_id_;
  }
...
public:
  ...
  const std::string& source() const throw()
  {
    return source_.get();
  }
...
public:
  explicit
  WebStatInnerData(unsigned long count = 0) throw()
  :
    count_(count)
  {
  }

  bool operator==(const WebStatInnerData& rhs) const throw()
  {
