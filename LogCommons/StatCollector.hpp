...
public:
  ...
  add(const ValueT& value)
  {
    ...
    auto ins_res = map_impl_->insert(value);
    if (!ins_res.second)
    {
...
private:
  ...
  {
  private:
    virtual ~RefCountableMapImpl_() throw() {}
  };
...
operator>>(
  ...
  throw(eh::Exception,
    typename StatCollector<KEY_, DATA_, EX_, U_F_, S_, O_>::Exception)
{
  collector.load(is);
  return is;
}
...
private:
  ...
  private:
    ...
    void
    splice_(Holder& collector) throw()
    {
...
public:
  ...
  void
  merge(SeqCollector& collector) throw()
  {
