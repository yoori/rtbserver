  ...
  namespace LogProcessing
  {
    namespace
    {
      ...
      struct FileNameInfo
      {
        ...
        FileNameInfo() throw () : type(REGULAR_FILE)
        {}
      };
    ...
    FileReceiver::FileGuard::~FileGuard() throw ()
    {
      if (file_receiver_)
      {
    ...
    bool
    FileReceiver::empty() const throw ()
    {
      SyncPolicy::WriteGuard guard(files_lock_);
      return (oldest_not_in_files_.empty() && files_.empty());
    }

    FileReceiver::~FileReceiver() throw ()
    {}
    ...
    void
    FileReceiver::file_processed_(const char* file_name) throw ()
    {
      SyncPolicy::WriteGuard guard(files_lock_);
    ...
    bool
    FileReceiver::access_(const char* name) throw ()
    {
    ...
    private:
      ...
      public:
        ...
        virtual void
        on_top_changed(const std::string&) throw ()
        {
          Sync::ConditionalGuard guard(cond_);
          cond_.broadcast();
        }
    ...
    void
    FileThreadProcessor::fetch_files_() throw ()
    {
