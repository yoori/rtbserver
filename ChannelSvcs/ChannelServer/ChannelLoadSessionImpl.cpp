...
namespace ChannelSvcs
{
  ...
  struct Filter
  {
    Filter(unsigned int* begin, unsigned int* end, unsigned int count) throw()
      : begin_(begin), end_(end), count_(count) {};

    bool operator()(unsigned int id) throw();
  ...
  bool Filter::operator()(unsigned int id) throw()
  {
  ...
  ChannelLoadSessionImpl::~ChannelLoadSessionImpl() throw()
  {
  }
  ...
  size_t ChannelLoadSessionImpl::get_item_() throw()
  {
    RGuard_ lock(init_lock_);
    return index_;
  }

  void ChannelLoadSessionImpl::bad_(size_t index) throw()
  {
