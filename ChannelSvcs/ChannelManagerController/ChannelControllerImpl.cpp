...
ChannelControllerImpl::CSInfo::CSInfo() throw()
  : control_ref(0),
    proccontrol_ref(0),
    server_ref(0),
    update_ref(0),
...
void ChannelControllerImpl::calc_proxy_sums_(
  const xsd::AdServer::Configuration::MultiCorbaObjectRefType& refs,
  bool use_local_on_first_load) throw ()
{
  for(CSConnection::CSGroups::iterator it = connection_.servers.begin();
      it != connection_.servers.end(); ++it)
  {
...
ChannelControllerImpl::~ChannelControllerImpl() throw()
{
...
void
ChannelControllerImpl::check_() throw()
{
...
ChannelClusterControlImpl::ChannelClusterControlImpl(
  ChannelControllerImpl* controller) throw():
  delegate_(ReferenceCounting::add_ref(controller))
{
}

ChannelClusterControlImpl::~ChannelClusterControlImpl() throw ()
{
}
...
ChannelClusterControlImpl::is_alive() throw()
{
  try
  {
...
ChannelStatImpl::ChannelStatImpl(ChannelControllerImpl* controller) throw():
  delegate_(ReferenceCounting::add_ref(controller))
{
}
