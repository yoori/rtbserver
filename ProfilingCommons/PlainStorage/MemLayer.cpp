...
namespace PlainStorage
{
  ...
  WriteMemLayer::WriteBlockImpl::~WriteBlockImpl() throw()
  {
    ::free(content_);
  }
  ...
  WriteMemLayer::~WriteMemLayer() throw()
  {}
  ...
  unsigned long
  WriteMemLayer::area_size() const throw()
  {
  ...
  SizeType
  WriteMemLayer::block_data_size() const throw ()
  {
    return block_size_ - 2*sizeof(u_int32_t);
  }

  MemBlockIndex
  WriteMemLayer::max_block_index() const throw()
  {
    SyncPolicy::ReadGuard lock(blocks_lock_);
    if(blocks_.empty())
    {
