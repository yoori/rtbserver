...
namespace
{
  ...
  const RevenueDecimal BASE_PUB_IMP_REVENUE = RevenueDecimal::mul(
    BASE_PUB_IMP_SYS_REVENUE, PUB_CURRENCY_RATE, Generics::DMR_CEIL);

  // cost after impression for RT_ABSOLUTE case
  const RevenueDecimal PUB_IMP_REVENUE = RevenueDecimal::mul(
    PUB_IMP_SYS_REVENUE, PUB_CURRENCY_RATE, Generics::DMR_CEIL);
   ...
   */
  std::ios_base&
  add_error(std::ios_base& iosbase) throw ()
  {
   ...
   */
  int
  get_errors(std::ios_base& iosbase) throw ()
  {
    ...
    public std::map<AdServer::Commons::RequestId, Counter>
  {
      ...
      const ProcessingState& processing_state)
    {
      const RevenueDecimal& expected_pub_imp_revenue =
        (ri.expression == "TEST-ABSOLUTE" ? PUB_IMP_REVENUE : PUB_SHARE_REVENUE);
      const RevenueDecimal& expected_pub_imp_sys_revenue =
        (ri.expression == "TEST-ABSOLUTE" ? PUB_IMP_SYS_REVENUE : PUB_SHARE_SYS_REVENUE);
      ...
      {
        std::cerr << "impression - incorrect pub revenue (" <<
    ...
    process_custom_action(
      const RequestInfo& ri,
      const AdvCustomActionInfo& adv_custom_action_info) throw ()
    {
      SyncPolicy::WriteGuard lock(lock_);
  ...
  protected:
    virtual ~TestRequestActionProcessorImpl() throw()
    {}
  ...
  public:
    ...
    process_impression(
      ...
      throw(Exception)
    {
      //std::cout << "move notice operation for '" << impression_info.user_id << "'" << std::endl;
  ...
  protected:
    virtual ~CheckRequestOperationProcessor() throw() {}
...
struct OrderFun
{
    ...
    RequestInfoContainer* request_info_container) const
  {
    ...
    request_info.pub_revenue.impression =
      ...
      BASE_PUB_IMP_REVENUE : // will be overriden at impr track
      (revenue_type_share ? PUB_SHARE_REVENUE : PUB_IMP_REVENUE);
...
request_info_container_test()
{
  try
  {
    ...
    for(int i = 0; i <= 2; ++i)
    {
      ...
      if(i != 0)
      {
        // container should delegate all operations excluding requests (request can be processed localy or delegated)
