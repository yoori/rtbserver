...
struct TestBase: public virtual ReferenceCounting::DefaultImpl<>
{
  ...
  virtual void fill_etalon(
    PassbackInfoList& passback_info_list) throw() = 0;

  static void print_passback_list(
    std::ostream& out,
    const PassbackInfoList& lst,
    const char* offset) throw()
  {
    unsigned long i = 0;
...
struct VerificationOfNonExistsPassbackTest: public TestBase
{
  ...
  virtual void fill_etalon(PassbackInfoList& /*etalon*/) throw()
  {}
};
...
int
passback_container_test(const char* test_folder) throw ()
{
