...
namespace PlainStorage
{
  ...
  struct TestIndexSerializer
  {
    ...
    static SizeType
    size(const TestIndex& /*val*/) throw()
    {
      return sizeof(TestIndex);
    }

    void
    save(const TestIndex& val, void* buf, SizeType /*sz*/) const throw ()
    {
      *(TestIndex*)buf = val;
    }
