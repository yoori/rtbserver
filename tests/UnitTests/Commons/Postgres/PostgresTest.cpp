...
namespace AdServer
{
  namespace UnitTests
  {
    ...
    private:
      virtual ~ConnectionActionTask() throw() {};
      PostgresTest *test_; 
    ...
    void PostgresTest::log_(const char* message, unsigned int level) throw()
    {
      file_logger_->log(String::SubString(message), level); 
    }

    void PostgresTest::generate_record_(Record& rec) throw()
    {
    ...
    std::string PostgresTest::generate_file_name_() throw()
    {
      ...
      throw(Exception)
    {
      ...
      char symb;
      if (my_file.read(&symb, 1) != 0 || !my_file.eof())
      {
        my_file.close();
        Stream::Error err;
    ...
    int PostgresTest::lob_test() throw()
    {
      int ret_value = 0;
    ...
    int PostgresTest::object_test() throw ()
    {
      int ret_value = 0;
      Commons::Postgres::Environment_var env;
      Commons::Postgres::Connection_var con;
    ...
    int PostgresTest::numeric_test() throw()
    {
      int ret_value = 0;
    ...
    void PostgresTest::query_perf_one_row(
      AdServer::Commons::Postgres::Connection* con) throw()
    {
    ...
    int PostgresTest::query_perf_test() throw()
    {
      int ret_value = 0;
      AdServer::Commons::Postgres::Connection_var con;
      try
    ...
    int PostgresTest::active_query_test() throw()
    {
      int ret_value = 0;
    ...
    int PostgresTest::connection_test() throw()
    {
      try
      {
        AdServer::Commons::Postgres::Connection_var con =
    ...
    int PostgresTest::environment_test() throw()
    {
      try
      {
        log_("Creating environment");
    ...
    int PostgresTest::pool_test() throw()
    {
      int res = 0;
      try
      {
    ...
    void ConnectionActionTask::execute() throw()
    {
      test_->deactivate_env(env_);
    }

    void PostgresTest::deactivate_env(
      Commons::Postgres::Environment* env) throw()
    {
      log_("Start task");
    ...
    int PostgresTest::no_db_test() throw()
    {
