...
namespace Commons
{
  namespace Postgres
  {
    struct LobHolder
    {
      ...
      LobHolder(char* src = 0, size_t src_len = 0, bool buffer_own = true) throw();
      LobHolder(const LobHolder&) throw() = delete;
      LobHolder(LobHolder&& tmp) throw();
      LobHolder(CharArray&& tmp, size_t src_len) throw();
      ~LobHolder() throw();
    private:
    ...
    {
    public:
      ResultSet(Connection *conn, PGresult* res = 0) throw ();
      ...
      */
      ...
      bool is_null(int columm_number) const throw();
...
*/
    ...
    inline
    LobHolder::LobHolder(char* src, size_t src_len, bool buffer_own) throw()
    ...
    inline
    LobHolder::LobHolder(CharArray&& tmp, size_t src_len) throw()
      : data_(tmp.release()),
        buffer(data_.get()),
        length(src_len)
    {
    ...
    inline
    LobHolder::LobHolder(LobHolder&& lob) throw()
