...
namespace Commons
{
  namespace Postgres
  {
    ...
    void Connection::terminate_i_() throw()
    {
      if(conn_)
      {
        PQfinish(conn_);
    ...
    void Connection::cancel_i_() throw ()
    {
    ...
    void Connection::bad() throw()
    {
      bad_ = true;
    }

    bool Connection::is_free() const throw()
    {
      return (ref_count_ == 1);
    }

    bool Connection::is_bad() const throw()
    {
      return (bad_ == true);
    }
    ...
    void Connection::clear_result_() throw()
    {
      if (have_result_set_)
      {
        PGresult* res_pg;
