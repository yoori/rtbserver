...
namespace Algs
{
  ...
  {
  public:
    ConstPointerHashAdapter(const Type* ptr) throw()
      : ptr_(ptr)
    {}
    ...
    const Type* value() const throw()
    {
      return ptr_;
    }

    bool operator==(const ConstPointerHashAdapter& right) const throw()
    {
  ...
  inline
  Generics::Time
  round_to_day(const Generics::Time& time) throw ()
  {
