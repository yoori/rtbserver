...
namespace Oracle
{
  ...
  {
    virtual ~ParamCommonHolder() throw() {}
    sb2 indicator;
    ub2 data_len;
  };
  ...
  {
    virtual ~ParamStringHolder() throw() {}
    std::string oci_text;
  };

  struct ParamLobHolder: public ParamCommonHolder
  {
    ParamLobHolder(const Lob& lob_val) throw()
      : lob(lob_val)
    {}

    virtual ~ParamLobHolder() throw() {}
  ...
  {
    ParamArrayHolder(Statement* stmt) throw()
      : statement(stmt),
        oci_named_type(
    ...
    virtual ~ParamArrayHolder() throw() {}
  ...
  {
    ParamStreamHolder(Statement* stmt) throw()
      : statement(stmt),
        oci_named_type(
          statement->connection_->environment_->environment_handle_.get(),
          statement->error_handle_.get()),
    ...
    virtual ~ParamStreamHolder() throw() {}
...
*/
  ...
  public:
    ...
    virtual ~OCIObjectFillCache() throw()
    {
...
//      std::cerr << "In: " << (void*)cur_oci_type << std::endl;
  ...
  void* SqlStream::object_ind_() throw()
  {
    return obj_ind_;
  }
