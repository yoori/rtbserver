...
namespace AdServer
{
  ...
  namespace
  {
    ...
    public:
      ...
      virtual ~StringEqualParamProcessor() throw() {}
      ...
      if(!request_info.referer.empty())
      {
           ...
           request_info.search_engine_id != 0)
        {
          try
          {
            ...
            // for SiteReferrerStats full can't be logged
            if(log_referrer_setting_ == Commons::LogReferrer::LR_FULL ||
              log_referrer_setting_ == Commons::LogReferrer::LR_PATH)
            {
