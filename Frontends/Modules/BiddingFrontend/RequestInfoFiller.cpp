...
namespace Request
{
  ...
  namespace OpenRtb
  {
    ...
    // user
    const String::SubString USER_USER_ID("id");
    const String::SubString USER_BUYER_ID("buyeruid");
    ...
    const String::SubString USER_EXT("ext");
    const String::SubString USER_EXT_EIDS("eids");
    const String::SubString USER_EXT_EIDS_UIDS("uids");
    const String::SubString USER_EXT_EIDS_UIDS_ID("id");
    const String::SubString USER_EXT_EIDS_UIDS_EXT("ext");
    const String::SubString USER_EXT_EIDS_UIDS_EXT_RTIPARTNER1("rti_partner");
    const String::SubString USER_EXT_EIDS_UIDS_EXT_RTIPARTNER2("rtiPartner");
...
namespace RTI
{
  const std::string PARTNER_TDID("tdid");
}
...
namespace Bidding
{
  ...
  protected:
    virtual
    ~JsonStringArrayParamProcessor() throw ()
    {}
  };
  ...
  protected:
    virtual ~JsonSerializeParamProcessor() throw()
    {}
  ...
  // JsonEIDSUIDSParamProcessor object processor
  class JsonEIDSUIDSParamProcessor:
    public JsonParamProcessor<JsonProcessingContext>
  {
    typedef JsonProcessingContext::EIDUID EIDUID;

    typedef JsonParamProcessor<EIDUID> JsonEIDUIDParamProcessor;

    typedef ReferenceCounting::SmartPtr<JsonEIDUIDParamProcessor>
      JsonEIDUIDParamProcessor_var;

  public:
    JsonEIDSUIDSParamProcessor()
    {
      typedef ReferenceCounting::SmartPtr<
        JsonCompositeParamProcessor<EIDUID> >
        JsonEIDUIDCompositeParamProcessor_var;
      
      JsonEIDUIDCompositeParamProcessor_var eid_uid_processor =
        new JsonCompositeParamProcessor<EIDUID>();

      eid_uid_processor->add_processor(
        Request::OpenRtb::USER_EXT_EIDS_UIDS_ID,
        JsonEIDUIDParamProcessor_var(
          new JsonContextStringParamProcessor<EIDUID>(
            &EIDUID::id)));

      JsonEIDUIDCompositeParamProcessor_var eid_uid_ext_processor =
        new JsonCompositeParamProcessor<EIDUID>();

      eid_uid_ext_processor->add_processor(
        Request::OpenRtb::USER_EXT_EIDS_UIDS_EXT_RTIPARTNER1,
        JsonEIDUIDParamProcessor_var(
          new JsonContextStringParamProcessor<EIDUID>(
            &EIDUID::rti_partner)));
      
      eid_uid_ext_processor->add_processor(
        Request::OpenRtb::USER_EXT_EIDS_UIDS_EXT_RTIPARTNER2,
        JsonEIDUIDParamProcessor_var(
          new JsonContextStringParamProcessor<EIDUID>(
            &EIDUID::rti_partner)));

      eid_uid_processor->add_processor(
        Request::OpenRtb::USER_EXT_EIDS_UIDS_EXT,
        eid_uid_ext_processor);

      eid_uid_processor_ = eid_uid_processor;
    }
      ...
      const JsonValue& value) const
    {
      if(value.getTag() == JSON_TAG_ARRAY)
      {
        for(JsonIterator it = begin(value); it != end(value); ++it)
        {
          process_eid_uid_(request_params, context, it->value);
        }
      }
      else
      {
        process_eid_uid_(request_params, context, value);
      }
    }

  protected:
    virtual ~JsonEIDSUIDSParamProcessor() throw()
    {}
  ...
  private:
    ...
    {
      EIDUID eid_uid;
      eid_uid_processor_->process(request_params, eid_uid, value);
      context.external_user_ids.push_back(std::move(eid_uid));
    }
 
  protected:
    ReferenceCounting::SmartPtr<
      JsonParamProcessor<EIDUID> > eid_uid_processor_;
  };
  ...
  protected:
    virtual ~JsonAssetProcessor() throw() = default;
    ...
    public JsonParamProcessor<JsonAdSlotProcessingContext>
  {
    ...
    typedef JsonCompositeParamProcessor<Native>
      JsonRequestCompositeParamProcessor;

    typedef ReferenceCounting::SmartPtr<JsonRequestCompositeParamProcessor>
      JsonRequestCompositeParamProcessor_var;

  public:
    JsonNativeProcessor()
    {
      JsonRequestCompositeParamProcessor_var processor =
        new JsonRequestCompositeParamProcessor();      

      JsonRequestCompositeParamProcessor_var native_processor =
        new JsonRequestCompositeParamProcessor();
      ...
      {
        // Special MobFox case
        JsonRequestCompositeParamProcessor_var mobfox_processor =
          new JsonRequestCompositeParamProcessor();
  ...
  private:
    ...
    add_native_processor_(
      JsonRequestCompositeParamProcessor* native_processor)
    {
      native_processor->add_processor(
  ...
  protected:
    virtual
    ~InventoryAuditsParamProcessor() throw ()
    {}
  };
      ...
      account_traits_(account_traits)
  {
...

    {
      ...
      user_processor->add_processor(
        Request::OpenRtb::USER_USER_ID,
        JsonRequestParamProcessor_var(
          new JsonContextStringParamProcessor<JsonProcessingContext>(
            &JsonProcessingContext::external_user_id)));

      user_processor->add_processor(
        Request::OpenRtb::USER_BUYER_ID,
        JsonRequestParamProcessor_var(
          new JsonContextStringParamProcessor<JsonProcessingContext>(
            &JsonProcessingContext::user_id)));
...

      {
        ...
        JsonCompositeRequestParamProcessor_var user_ext_eid_processor =
          new JsonCompositeRequestParamProcessor();

        user_ext_eid_processor->add_processor(
          Request::OpenRtb::USER_EXT_EIDS_UIDS,
          JsonRequestParamProcessor_var(
            new JsonArrayParamProcessor<JsonProcessingContext>(
              JsonRequestParamProcessor_var(new JsonEIDSUIDSParamProcessor()))));

        ReferenceCounting::SmartPtr<
          JsonArrayParamProcessor<JsonProcessingContext> >
          user_ext_eids_processor = new JsonArrayParamProcessor<JsonProcessingContext>(
            user_ext_eid_processor);

        user_ext_processor->add_processor(
          Request::OpenRtb::USER_EXT_EIDS,
          user_ext_eids_processor);

        user_processor->add_processor(
          Request::OpenRtb::USER_EXT,
          user_ext_processor);
      }
  ...
  bool
  RequestInfoFiller::fill_adid(const RequestInfo& request_info) const throw()
  {
    ...
    throw(InvalidParamException, Exception)
  {
    ...
    {
      kw_fmt.add_dict_keyword(MatchKeywords::CARRIER, norm_keyword_(context.carrier));
    }
        ...
        slot_it != context.ad_slots.end(); )
    {
          ...
          debug_sizes.empty() && default_debug_size.empty()))
      {
        ...
        else // banners not empty here
        {
          ...
          ad_slot_request.sizes.length(slot_it->banners.size());
              ...
              banner_it != slot_it->banners.rend(); ++banner_it)
          {
            ...
            if(!skip_sizes)
            {
              ...
              {
                // 2x2 is overlay and must be return as "script with url"
                ad_slot_request.sizes.length(1);
                ad_slot_request.sizes[0] << RM_SIZE;
                size_i = 1;

                request_params.ad_instantiate_type = AdServer::CampaignSvcs::AIT_SCRIPT_WITH_URL;
                skip_sizes = true;

                if(request_params.common_info.request_type ==
                   AdServer::CampaignSvcs::AR_YANDEX)
                {
                  slot_it->size_banner.clear();
                  slot_it->size_banner.insert(std::make_pair(RM_SIZE, *banner_it));
                }
              }
              else if(banner.ext_type == "20")
              {
                ad_slot_request.sizes[size_i++] << RM_SIZE;
                slot_it->size_banner.insert(std::make_pair(RM_SIZE, *banner_it));
              }
              else if (banner.ext_format == "overlay")
              {
                ad_slot_request.sizes[size_i++] << RM_DTO_SIZE;
                slot_it->size_banner.insert(std::make_pair(RM_DTO_SIZE, *banner_it));
              }
              else if (banner.ext_format == "rich")
              {
                ad_slot_request.sizes[size_i++] << RM_RICH_SIZE;
                slot_it->size_banner.insert(std::make_pair(RM_RICH_SIZE, *banner_it));
              }
              else
              {
                const DebugAdSlotSizeMap& debug_sizes = request_info.debug_sizes;
                ...
                std::string res_size = (size_it != debug_sizes.end() ? size_it->second : (
                  request_info.default_debug_size.empty() ? banner.width + "x" + banner.height :
                  request_info.default_debug_size) );
                ad_slot_request.sizes[size_i++] << res_size;
                slot_it->size_banner.insert(std::make_pair(res_size, *banner_it));
              }
            }
          }
        ...
        {
          String::AsciiStringManip::to_lower(*context.currencies.begin());
        }

        if(!context.currencies.empty())
        {
          ad_slot_request.currency_code << *context.currencies.begin();
        }
        else if(!slot_it->min_cpm_price_currency_code.empty())
        {
          ad_slot_request.currency_code << slot_it->min_cpm_price_currency_code;
        }
    ...
    for(auto eid_uid_it = context.external_user_ids.begin();
      eid_uid_it != context.external_user_ids.end(); ++eid_uid_it)
    {
      String::AsciiStringManip::to_lower(eid_uid_it->rti_partner);

      if(eid_uid_it->rti_partner == RTI::PARTNER_TDID && !eid_uid_it->id.empty())
      {
        request_info.external_user_ids.push_back(
          std::string("tdid/") + eid_uid_it->id);
      }
    }
  ...
  RequestInfoFiller::init_request_param_(
    AdServer::CampaignSvcs::CampaignManager::RequestParams& request_params,
    RequestInfo& request_info) const throw()
  {
    CorbaAlgs::fill_sequence(
  ...
  std::string
  RequestInfoFiller::norm_keyword_(const String::SubString& kw) throw()
  {
