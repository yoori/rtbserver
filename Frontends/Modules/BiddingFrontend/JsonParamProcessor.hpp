...
namespace Bidding
{
  ...
  struct JsonAdSlotProcessingContext
  {
    ...
    {
      Banner()
        : pos("0"), ext_hpos(CampaignSvcs::UNDEFINED_PUB_POSITION_BOTTOM)
      {}
  ...
  struct JsonProcessingContext
  {
    ...
    {
      std::string id;
      std::string rti_partner;
    };

    typedef std::vector<EIDUID> EIDUIDArray;
    ...
    print(std::ostream& out) const
    {
          ...
          slot_it != ad_slots.end(); ++slot_it)
      {
        ...
        {
          out << "{ width = " << (*banner_it)->width <<
            ", height = " << (*banner_it)->height <<
              ", pos = " << (*banner_it)->pos <<
  ...
  protected:
    virtual ~JsonParamProcessor() throw()
    {}
  };
  ...
  protected:
    virtual ~JsonCompositeParamProcessor() throw()
    {}
  ...
  protected:
    virtual ~JsonArrayParamProcessor() throw()
    {}
  ...
  public:
    JsonContextStringParamProcessor(
      std::string ContextType::* field,
      bool override_on_empty = false)
      : field_(field),
        override_on_empty_(override_on_empty)
    {}
      ...
      const JsonValue& value) const
    {
         ...
         value.getTag() == JSON_TAG_STRING)
      {
        std::string val;
        value.toString(val);
        if(override_on_empty_ || !val.empty())
        {
          (context.*field_).swap(val);
        }
      }
    }

  protected:
    virtual ~JsonContextStringParamProcessor() throw()
    {}

  protected:
    std::string ContextType::* field_;
    const bool override_on_empty_;
  };
  ...
  protected:
    virtual ~JsonContextNumberParamProcessor() throw()
    {}
  ...
  protected:
    virtual ~JsonContextNumberStateParamProcessor() throw()
    {}
  ...
  protected:
    virtual ~JsonContextBoolParamProcessor() throw()
    {}
  ...
  protected:
    virtual ~JsonContextDoubleParamProcessor() throw()
    {}
  ...
  protected:
    virtual ~JsonContextURLParamProcessor() throw()
    {}
  ...
  protected:
    virtual
    ~JsonContextNumberArrayParamProcessor() throw ()
    {}
  };
  ...
  protected:
    virtual
    ~JsonContextNumberArrayStateParamProcessor() throw ()
    {}
  };
  ...
  protected:
    virtual
    ~JsonContextStringArrayParamProcessor() throw ()
    {}
  };
}
}
