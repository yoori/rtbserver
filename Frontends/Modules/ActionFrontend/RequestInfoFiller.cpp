...
namespace Action
{
    ...
    throw(InvalidParamException, ForbiddenException, Exception)
  {
      ...
      if(request_info.user_status != AdServer::CampaignSvcs::US_OPTOUT)
      {
        ...
        else if(set_uid_)
        {
          ...
          bool pass_by_country =
            (!country_filter.in() ||
             country_filter->enabled(request_info.country)) ?
            true : false;
    ...
    // process case when external_user_id is UNSIGNEDUID or UNSIGNEDUID/UNSIGNEDCOOKIEUID
    ...
    request_info.full_referer = request_info.referer;

    // shrink referer for precision from config
    if(use_referrer_ != Commons::LogReferrer::LR_FULL)
    {
    ...
    throw()
  {
    ...
    else
    {
      ...
      if(!u2.empty())
      {
        try
        {
          request_info.utm_cookie_user_id = Commons::UserId(u2);
        }
        catch(const eh::Exception&)
        {
          // ignore invalid user ids in utm_term
