...
namespace AdServer
{
  ...
  bool
  ClickFrontend::will_handle(const String::SubString& uri) throw ()
  {
    ...
    if (!peer_ip_val.empty() && ip_map_.get())
    {
      try
      {
             ...
             false))
        {
          ...
          // FIXME: 
          // copy_location in FrontendCommons/GeoInfoUtils.hpp?
          mri.match_info.location.length(1);
          mri.match_info.location[0].country << location->country;
          mri.match_info.location[0].region << location->region;
          mri.match_info.location[0].city << location->city;
        }
      }
