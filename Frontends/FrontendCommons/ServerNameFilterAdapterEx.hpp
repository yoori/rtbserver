...
namespace Apache
{
  ...
  public:
    ServerNameFilterAdapterEx(int where = APR_HOOK_LAST) throw();

    void
    add_module_server(const server_rec* server) throw();

    bool
    module_used() const throw();

    virtual int
    quick_handler(request_rec* r, int) throw();

    void
    add_prev(const server_rec* server, ServerAction* action) throw ();

    void
    add_post(const server_rec* server, ServerAction* action) throw ();
  ...
  protected:
    virtual
    ~ServerNameFilterAdapterEx() throw()
    {}
