...
namespace AdServer
{
  ...
  namespace RequestInfoSvcs
  {
    ...
    public:
      void
      inc_web_index_request_sent() throw ();

      const Stats&
      get_stats() const throw ();
    private:
      virtual
      ~StatsCounters() throw () = default;
    };
    ...
    public:
      ...
      LogStatValues_var
      get_log_values(unsigned log_index) throw ()
      {
        return stats_[log_index];
      }

      const Generics::Values_var&
      get_app_stats() const throw ()
      {
        return app_stats_;
      }

      const LogStatValues_var
      get_log_values(unsigned log_index) const throw ()
      {
        return stats_[log_index];
      }
    ...
    private:
      ...
      {
      public:
        FunctorWrapper(Functor& f, const char* log_name) throw ()
          : functor_(f),
            log_name_(log_name)
        {}
      ...
      virtual
      ~RequestInfoManagerStatsImpl() throw () {}
...
namespace AdServer
{
  namespace RequestInfoSvcs
  {
    ...
    inline void
    StatsCounters::inc_web_index_request_sent() throw ()
    {
