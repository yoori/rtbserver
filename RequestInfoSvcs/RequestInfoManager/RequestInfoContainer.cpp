...
namespace RequestInfoSvcs {
    ...
    bool notice)
  {
    ...
    if(request_writer.request_done())
    {
      ...
      {
      */
      if(true) // make this step always for reeval amounts by different division models
      {
        ...
        const RevenueDecimal pub_currency_rate(
          request_writer.pub_currency_rate());
        ...
        //bool dont_check_cost = request_writer.enabled_notice() & DISABLED_PUB_COST_CHECK;
        RevenueDecimal res_pub_revenue;

        if(!impression_info.pub_revenue.present() ||
          impression_info.pub_revenue->revenue_type ==
            AdServer::CampaignSvcs::RT_ABSOLUTE)
        {
          res_pub_revenue = imp_pub_revenue;
        }
        else // RT_SHARE
        {
          ...
          res_pub_revenue = RevenueDecimal::mul(
            orig_pub_revenue.impression,
            pub_share,
            Generics::DMR_CEIL);
        }

        /* FIXME - Unused result of division
        const RevenueDecimal res_pub_sys_revenue =*/ div(
          res_pub_revenue,
          pub_currency_rate,
          "Publisher currency rate is ZERO!",
          acc_id);

        res_pub_revenue = std::min(res_pub_revenue, orig_pub_revenue.impression);

        request_writer.pub_revenue().impression() = res_pub_revenue.str();
        ...
        if(!notice && (!request_writer.enabled_notice() || request_writer.notice_received()))
        {
          ...
          res_pub_revenue = RevenueDecimal::div(
            res_pub_revenue,
          ...
          request_writer.pub_comm_revenue().impression() =
            res_pub_comm_revenue.str();
...

          {
            ...
            /*
            std::cerr << "delta_adv_revenue: " << std::endl;
            delta_adv_revenue.print(std::cerr, "  ");

            std::cerr << "delta_adv_comm_revenue: " << std::endl;
            delta_adv_comm_revenue.print(std::cerr, "  ");

            //std::cerr << "delta_pub_revenue: " << std::endl;
            //delta_pub_revenue.print(std::cerr, "  ");

            std::cerr << "delta_isp_revenue: " << std::endl;
            delta_isp_revenue.print(std::cerr, "  ");
            */
          ...
          /*
            const RevenueDecimal adv_currency_rate(request_writer.adv_currency_rate());
            const RevenueDecimal imp_adv_revenue(request_writer.adv_revenue().impression());

            const RevenueDecimal imp_adv_sys_revenue = div(
              imp_adv_revenue,
              adv_currency_rate,
              "Advertiser currency rate is ZERO!",
              acc_id);

            const RevenueDecimal imp_pub_sys_revenue = div(
              res_pub_revenue,
              pub_currency_rate);

            const RevenueDecimal isp_currency_rate(request_writer.isp_currency_rate());

            const RevenueDecimal isp_revenue_share(request_writer.isp_revenue_share());

            res_isp_revenue = RevenueDecimal::mul(
              RevenueDecimal::mul(
                imp_adv_sys_revenue - imp_pub_sys_revenue,
                isp_currency_rate,
                Generics::DMR_CEIL),
              isp_revenue_share,
              Generics::DMR_CEIL);

            request_writer.isp_revenue().impression() = res_isp_revenue.str();
          */
        ...
        else if (res_pub_revenue > orig_pub_revenue.impression)
        {
          logger_->sstream(
            Logging::Logger::ERROR,
            Aspect::REQUEST_INFO_CONTAINER,
            "ADS-TF-3000") <<
              "Bid price is LOWER than Win price:" << std::endl <<
              "  Publisher ID: " << acc_id << std::endl <<
              "  Tag ID: " << request_writer.tag_id() << std::endl <<
              "  Request ID: " << impression_info.request_id << std::endl <<
              "  Bid price: " << orig_pub_revenue.impression.str() << std::endl <<
              "  Win price: " << res_pub_revenue.str() << std::endl;
        }
      } // if (impression_info.pub_revenue.present())
    }
  ...
  {
  public:
    ProxyImpl(RequestInfoContainer* owner) throw();
  ...
  protected:
    virtual
    ~ProxyImpl() throw () = default;

  private:
    typedef Sync::Policy::PosixThread SyncPolicy;

  private:
    RequestInfoContainer_var lock_owner_() const throw();
  ...
  public:
    ...
    RequestOperationProxy(RequestInfoContainer* owner) throw();
  ...
  protected:
    virtual
    ~RequestOperationProxy() throw () = default;
  ...
  void
  RequestInfoContainer::ProxyImpl::detach() throw()
  {
  ...
  RequestInfoContainer::ProxyImpl::lock_owner_() const throw()
  {
  ...
  void
  RequestInfoContainer::RequestOperationProxy::detach() throw()
  {
    SyncPolicy::WriteGuard lock(lock_);
    owner_ = 0;
  }
  ...
  RequestInfoContainer_var
  RequestInfoContainer::RequestOperationProxy::lock_owner_() const throw()
  {
    SyncPolicy::ReadGuard lock(lock_);
    return ReferenceCounting::add_ref(owner_);
  }
  ...
  RequestInfoContainer::~RequestInfoContainer() throw()
  {
    proxy_->detach();
  }

  RequestContainerProcessor_var
  RequestInfoContainer::proxy() throw()
  {
    return proxy_;
  }

  RequestOperationProcessor_var
  RequestInfoContainer::request_operation_proxy() throw()
  {
    ...
    throw(RequestContainerProcessor::Exception)
  {
    ...
    try
    {
      RequestInfoMap::Transaction_var transaction =
        request_map_->get_transaction(request_info.request_id);
    ...
    throw(RequestContainerProcessor::Exception)
  {
    ...
    {
      Generics::ConstSmartMemBuf_var mem_buf =
        request_map_->get_profile(request_id);
    ...
    throw(RequestContainerProcessor::Exception)
  {
    ...
    {
      RequestInfoMap::Transaction_var transaction =
        request_map_->get_transaction(impression_info.request_id);
    ...
    throw(Exception)
  {
    ...
    try
    {
...

      {
        ...
        impression_non_considered = new_request_reader.impression_non_considered();
        ...
        {
          click_non_considered += 1;
          request_writer.click_done() = 0;
        }

        click_non_considered += request_writer.click_non_considered();
...
#       ifdef DEBUG_OUTPUT_
        std::cerr << "RequestInfoContainer::change_request_user_id_(): "
          ...
          request_writer.click_non_considered() << std::endl;
#       endif
...
#         endif
          ...
          {
            // don't use profile if it marked as RS_MOVED : it is local move
            if(request_writer.notice_non_considered() == 0 &&
               old_request_reader.notice_non_considered() != 0)
            {
              request_writer.notice_non_considered() = 1;
              convert_request_reader_to_impression_info(
                notice_info,
            ...
            // save impression_non_considered for process duplicates
            impression_non_considered +=
              old_request_reader.impression_non_considered();

            click_non_considered += old_request_reader.click_non_considered();
      ...
      bool request_processed_on_sender =
        request_reader.fraud() != RequestInfo::RS_NORMAL;
  ...
  RequestInfoContainer::get_profile_(
    RequestInfoMap::Transaction* transaction)
  {
