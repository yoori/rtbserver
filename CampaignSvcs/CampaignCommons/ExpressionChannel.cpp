...
namespace CampaignSvcs
{
  ...
  ReferenceCounting::SmartPtr<ExpressionChannelBase>
  ExpressionChannel::optimize()
  {
    ...
    return ReferenceCounting::SmartPtr<ExpressionChannelBase>();
  }

  bool
  ExpressionChannel::equal(const ExpressionChannel* right) const throw()
  {
    ...
    throw (eh::Exception)
  {
    ...
    else if (channel->simple_channel())
    {
      ...
      expr_.channel_id = channel->simple_channel()->params().channel_id;
    }
  ...
  bool
  FastExpressionChannel::has_params() const throw()
  {
    return false;
  }

  const ChannelParams&
  FastExpressionChannel::params() const throw()
  {
    return EMPTY_CHANNEL_PARAMS;
  }

  ChannelParams&
  FastExpressionChannel::params() throw()
  {
    return EMPTY_CHANNEL_PARAMS;
  }
    ...
    const ChannelIdHashSet& channels)
  {
    ...
    case ExpressionChannel::NOP:
      result = (channels.find(expr.channel_id) != channels.end());
      break;
    ...
    case ExpressionChannel::OR:
      {
        /*
        auto it1 = expr.simple.cbegin();

        while (it1 != expr.simple.cend())
        {
          const auto it2 = channels.lower_bound(*it1);

          if (it2 == channels.cend())
          {
            break;
          }
          else if (*it1 == *it2)
          ...
          it1 = std::lower_bound(it1, expr.simple.cend(), *it2);
        }
        */
