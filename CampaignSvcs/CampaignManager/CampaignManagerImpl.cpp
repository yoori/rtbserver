...
namespace AdServer
{
  namespace CampaignSvcs
  {
    ...
    public:
      ...
      const Creative* search_ccid(unsigned long ccid) throw()
      {
    ...
    CampaignManagerImpl::AdSlotMinCpm::AdSlotMinCpm() throw ()
      : min_pub_ecpm(RevenueDecimal::ZERO),
        min_pub_ecpm_system(RevenueDecimal::ZERO)
    {}

    /** CampaignManagerImpl::AdSlotContext */
    CampaignManagerImpl::AdSlotContext::AdSlotContext() throw ()
      : test_request(false),
    ...
    */
        ...
        if(ad_slot.currency_code[0])
        {
          for(CORBA::ULong i = 0; i < request_params.publisher_account_ids.length(); ++i)
          {
            ...
            if(account_it != campaign_config.accounts.end() && (
              ad_slot.currency_code[0] == 0 ||
              account_it->second->currency->currency_code == ad_slot.currency_code.in()))
            {
      ...
      throw(AdServer::CampaignSvcs::CampaignManager::ImplementationException)
    {
      ...
      try
      {
        ...
        {
          trace_stream << "  <message>can't find tag</message>" << std::endl;
        }
    ...
    void
    CampaignManagerImpl::flush_logs() throw()
    {
        ...
        AdServer::CampaignSvcs::CampaignManager::NotReady)
    {
      ...
      try
      {
        ...
        adv_action_info.country = action_info.country_code;
      ...
      throw()
    {
      ...
      try
      {
        ...
        RevenueDecimal adapted_min_ecpm = CorbaAlgs::unpack_decimal<RevenueDecimal>(min_ecpm);
        ...
        const RevenueDecimal min_pub_ecpm = RevenueDecimal::mul(
          adapted_min_ecpm,
          REVENUE_ONE + tag->cost_coef,
          Generics::DMR_FLOOR);
          ...
          if(creative && creative->campaign && creative->campaign->account.in())
          {
            ...
            const BillingStateContainer::BidCheckResult check_result =
              confirm_billing_state_container_->confirm_bid(
                ...
                cc_it->ctr,
                RevenueDecimal(false, rate_type == CR_CPM ? 1 : 0, 0),
    ...
    void CampaignManagerImpl::produce_ads_space_(
      const AdServer::CampaignSvcs::CampaignManager::
      ...
      unsigned long request_tag_id)
    {
      if (kafka_producer_ &&
        ...
        request_params.common_info.referer[0])
      {
        ...
        std::string user_id(
          CorbaAlgs::unpack_user_id(
            request_params.common_info.user_id).to_string(false));

        std::string csv_encoded_ref;
        String::StringManip::csv_encode(request_params.common_info.referer.in(), csv_encoded_ref);
        ...
        record += ',';
        record += request_params.common_info.peer_ip;

        kafka_producer_->push_data(
          user_id,
          record);
      }
    }
