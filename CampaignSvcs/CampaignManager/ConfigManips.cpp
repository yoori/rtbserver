...
namespace AdServer
{
  namespace CampaignSvcs
  {
    ...
    void
    CampaignManagerImpl::check_config() throw()
    {
      ...
      throw(Exception)
    {
      ...
      try
      {
        ...
        ConstCTRProvider_var new_ctr_provider;

        if(!old_ctr_provider)
        {
          new_config_timestamp = CTRProvider::check_config_appearance(
            new_ctr_config_root,
            capture_root);
        }
        ...
        if(!new_ctr_config_root.empty())
        {
            ...
            // for remove (will be reloaded or skipped at next interation)
          {
            // capture CTRConfig folder
            {
              ...
              if(::access(new_ctr_config_root_error.c_str(), F_OK) != 0)
              {
                if(!config_captured)
                {
                  ...
                  {
                    eh::throw_errno_exception<CTRProvider::InvalidConfig>(
                      "Can't rename file '",
                      new_ctr_config_root,
                      "' to '",
                      captured_ctr_config_root.c_str(),
            ...
            if(!new_ctr_config_root.empty())
            {
              try
              {
                new_ctr_provider = new CTRProvider(
                  new_ctr_config_root,
    ...
    void
    CampaignManagerImpl::update_ctr_provider() throw()
    {
    ...
    void
    CampaignManagerImpl::update_conv_rate_provider() throw()
    {
