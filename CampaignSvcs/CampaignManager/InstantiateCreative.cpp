...
namespace
{
  ...
  const char ADRIVER_MIME_CLICKPIXEL[] =
    "%2F%2F![rhost]%2Fcgi-bin%2Feclick.cgi%3Fxpid%3D![xpid]";
}
...
namespace AdServer
{
  namespace CampaignSvcs
  {
    ...
    const char* instantiate_user_status(UserStatus user_status) throw ()
    {
      ...
      throw (CreativeTemplateProblem, CreativeOptionsProblem, eh::Exception)
    {
      ...
      if(ad_instantiate_type == AIT_URL)
      {
        ...
        request_params.request_type == AR_GOOGLE)
      {
        ...
        if(source_rule_it != creative_instantiate_.source_rules.end())
        {
          if(!source_rule_it->second.mime_encoded_preclick.empty())
          {
            url_ostr << "&preclick=" << source_rule_it->second.mime_encoded_preclick;
