...
public:
  ...
  ThreadLogSaverImpl(
    const LogProcThreadInfo_var& context,
    const LogSaverBase_var& saver) throw () :
    context_(context),
  ...
  dump_start(
    const Spillover_var& data,
    CollectorBundleType* spillovers_owner = 0) throw ()
  {
    try
    {
  ...
  virtual void
  dump_portion(const Spillover_var& data) throw ()
  {
    {
      LogProcThreadInfo::ConditionalGuardT
        guard(context_->upload_event, context_->upload_event_mutex);
