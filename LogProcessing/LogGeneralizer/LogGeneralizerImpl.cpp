...
void
LogGeneralizerImpl::check_logs(const LogProcThreadInfo_var& context) throw ()
{
  try
  {
...
void
LogGeneralizerImpl::clear_expired_hits_data() throw()
{
  {
    Stream::Error es;
    es << FNS << "Enter";
...
void
LogGeneralizerImpl::schedule_clear_expired_msg() throw()
{
  {
    Stream::Error es;
    es << FNS << "Enter";
...
void
LogGeneralizerImpl::disable_upload_() throw ()
{
...
template <typename LogTraits>
void
LogGeneralizerImpl::enable_upload_() throw ()
{
  const LogProcThreadInfo_var& context(ProcessingContexts::get<LogTraits>());
  if (context)
  {
...
void
LogGeneralizerImpl::MoveLogsTask::execute() throw ()
{
