...
namespace
{
  ...
  public:
    ...
    FreqCapWriter
    operator()(const FreqCapCounter& fc_counter) throw()
    {
  ...
  {
  public:
    FreqCapMerger(const Generics::Time& now) throw()
      : now_(now)
    {}
  ...
  {
    bool operator()(const FreqCapWriter& val) const throw()
    {
      return val.fc_id() != 0;
    }
  };
  ...
  {
    bool operator<(const FreqCapDecrease& right) const throw()
    {
  ...
  struct SeqOrderAdapter
  {
    ...
    SeqOrderWriter
    operator()(const UserFreqCapProfile::SeqOrder& seq_order) throw()
    {
  ...
  read_campaign_freqs(
    ...
    const UserFreqCapProfileReader& reader) throw()
  {
  ...
  clean_up(
    UserFreqCapProfileWriter& profile,
    const Generics::Time& low_bound) throw()
  {
